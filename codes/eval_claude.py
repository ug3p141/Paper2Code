...
from claude_utils import print_log_cost_claude, claude_api_call
...
def main(args):
    ...
    # For Claude, we need to handle multiple generations differently
    # Claude doesn't support n parameter like OpenAI, so we'll make multiple calls
    all_scores = []
    rationales = []
    ...
    for i in range(generated_n):
        ...
        completion = claude_api_call(client, msg, claude_model, max_tokens=4096)
        
        # Calculate cost for this call
        temp_total_accumulated_cost = print_log_cost_claude(completion, claude_model, f"[Evaluation] {paper_name} - {eval_type} ({i+1}/{generated_n})", output_dir, total_accumulated_cost)
    ...
    # Create a mock completion_json for compatibility
    completion_json = {
        "choices": [{"message": {"content": f"Generated {len(all_scores)} evaluations"}}],
        "usage": {
            "input_tokens": 0,  # This would need to be calculated if needed
            "output_tokens": 0,
            "total_tokens": 0
        }
    }
    ...
    with open(f"{eval_result_dir}/{paper_name}_eval_{eval_type}_{claude_model.replace('-', '_')}_{now_str}.json", 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--claude_model', type=str, default="claude-sonnet-4-20250514")
